...
Itunes.prototype.validateTwoFactor = function(securityCode, deviceIdentifier) {
  ...
  return new Promise(function (resolve, reject){
    ...
    */
    ...
    // TODO: check for trusted device list
    request.post(url, params, function(error, response, body) { 
      ...
      // request.get("https://idmsa.apple.com/appleauth/auth/2sv/trust", {headers:headers}, function (error, response, body) {
      // self._cookies = cookies.map(e => e.toString().split(";", 1)[0]).join("; ");
      self.handleLogin(error, response, body).then(r => { 
        resolve(r)
      }).catch(e => {
        reject(e)
